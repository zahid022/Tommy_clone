import { FaAngleDown } from "react-icons/fa";

interface filterType {
    txt: string;
    icon: boolean;
    setFlag: (value: boolean) => void;
}

function FilterSelect({ txt, icon, setFlag }: filterType) {
    return (
        <div onClick={() => setFlag(true)} className="flex cursor-pointer items-center text-[#00174f] gap-2 px-4 border border-[#e5e5e5] rounded-[50px] h-[37px] ">
            <p className="text-[14px]">{txt}</p>
            <span>
                {icon ? <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={20}
                    height={20}
                    viewBox="0 0 24 24"
                    fill="none"
                >
                    <path
                        d="M3 7H6M6 7C6 8.65685 7.34315 10 9 10C10.6569 10 12 8.65685 12 7C12 5.34315 10.6569 4 9 4C7.34315 4 6 5.34315 6 7ZM3 17H9M18 17H21M18 17C18 18.6569 16.6569 20 15 20C13.3431 20 12 18.6569 12 17C12 15.3431 13.3431 14 15 14C16.6569 14 18 15.3431 18 17ZM15 7H21"
                        stroke="#00174f"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                    />
                </svg> : <FaAngleDown />}
            </span>
        </div>
    )
}

export default FilterSelect